...
/*
 * __to_another_endian(from, to, bit_array, size)
 * from/to : enum endianness, if you specify 'from' as 0, it will be changed to __ENDIAN(your processor's endian)
 * bit_array : pointer to a storage that you want to rearrange
 */
#define __impl_to_another_endian(from, to, bit_array, size)\
{\
	__ct_assert( !(size&1) &&\
			((from)<=3 && (from)>=1) &&\
			( ((to)<=3 && (to)>=1) ) )\
		...
		switch (to) {\
			case LITTLE_ENDIAN:\
				if (from == BIG_ENDIAN) {\
					...
					for (_i=0, _j=((size)>>1)-1; _i<_j; _i++, _j--) {\
			...
			case BIG_ENDIAN:\
				if (from == LITTLE_ENDIAN) {\
					...
					for (_i=0, _j=1; _j<(size); _i=++_j, _j++) {\
						__swap((uint8_t*)(bit_array)+_i, (uint8_t*)(bit_array)+_j)\
					}\
				}\
				break;\
...
#define __to_another_endian(from, to, bit_array, size)\
	__impl_to_another_endian((from?from:__ENDIAN), to, bit_array, size)
